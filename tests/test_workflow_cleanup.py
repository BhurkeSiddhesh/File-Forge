...
never deleted — they piled up in OUTPUT_DIR until the periodic stale-file sweep,
...
def test_multistep_chain_leaves_only_deliverable(mock_dirs, auth_client, tmp_path):
    ...
    output_files = sorted(p.name for p in mock_dirs["output"].iterdir())
    # Exactly the deliverable, nothing else. Before the fix this was 3 files
    # (deliverable + 2 orphaned uuid-prefixed intermediates).
    assert output_files == [deliverable], output_files
    assert (mock_dirs["output"] / deliverable).exists()
...
def test_failed_chain_cleans_intermediates(mock_dirs, auth_client, tmp_path):
    ...
    # intermediate (which leaked before the fix) must be gone.
